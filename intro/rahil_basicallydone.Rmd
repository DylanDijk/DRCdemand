...
estimate <- function(sumobj, modelsclusttype, cluster){
  ...
  for (i in (1:48)){
    post <- extract(modelsclusttype[[cluster]][[i]])
...
clusterEvalQ(cl, c(library(rstan), library(data.table), library(rstanarm)))

clusterExport(cl, c('dataprocess','estimate','fitmodel','stan_code','sumdo', 'parfit'))

clustmodeltest <- parLapply(cl,1:4, function(x){parfit(sumdo, x)})

stopCluster(cl)
```

```{r}
p <- estimate(sumdo, clustmodeltest, 4)
```
```{r}
pl <- plotpred(p, 1)
```
